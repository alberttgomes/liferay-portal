...
UpgradeJavaFetchCommerceAccountGroupByExternalReferenceCodeCheck | .bnd, .ftl, .gradle, .java, .json, .jsp, .jspf, .scss or .vm | Replace method and reorder parameters on method `fetchCommerceAccountGroupByExternalReferenceCode` from `AccountGroupLocalService` |
